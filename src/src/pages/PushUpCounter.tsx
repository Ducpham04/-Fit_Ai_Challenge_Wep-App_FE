...
import { uploadVideoForAnalysis, PushUpAnalysisResult } from '../api/pushUpAnalysis';
...
export const PushUpCounter = () => {
  ...
  const [backendResult, setBackendResult] = useState<PushUpAnalysisResult | null>(null);
  ...
  useEffect(() => {
    const fetchDailyTarget = async () => {
      try {
        ...
        console.log('📊 Fetching daily push-up target from API...');
        
        // Fetch the current day's target from the API
        const target = await workoutPlanApi.getCurrentDayTarget('intermediate');
        
        setDailyTarget(target);
        setTargetReps(target.pushUps);
        setCurrentDay(target.day);
        
        console.log(`✅ Daily target loaded: Day ${target.day} - ${target.pushUps} push-ups`);
      } catch (err: any) {
        ...
        setTargetReps(10);
        console.log('⚠️ Using fallback target: 10 push-ups');
      } finally {
  ...
  const handlePlayStateChange = useCallback((isPlaying: boolean) => {
    ...
    // Note: Don't call startProcessing/stopProcessing here!
    // The useEffect below will handle this automatically based on the state change
  }, []);
  ...
  useEffect(() => {
    const submitToBackend = async () => {
      if (metrics.reps >= targetReps && !hasSubmittedRef.current && videoFile && !isCompleted) {
        console.log(`🎯 Target of ${targetReps} reps reached! Submitting to backend...`);
  ...
  const handleReset = useCallback(() => {
    ...
    // 2. Reset the video to beginning (this will pause and set isVideoPlaying to false)
    if (videoPlayerRef.current) {
      videoPlayerRef.current.resetVideo();
      console.log('  ✓ Video reset to beginning');
    }
    
    // 3. Reset all metrics to initial values (after video to ensure clean state)
    resetCounter();
    ...
    // Note: When user clicks play again, the useEffect above will
    // automatically call startProcessing() because:
    // - isModelReady = true (already loaded)
    // - videoElement = exists
    // - isVideoPlaying = true (when they click play)
    // - isProcessing = false (we just stopped it)
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-700 mb-2">
                        You've done {targetReps} push-ups! Great job!
                      </p>
              ...
              {isLoadingTarget ? (
                ...
                <div className="bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-medium text-purple-600 uppercase">Day {currentDay} Target</p>
                      <p className="text-2xl font-bold text-purple-900">{targetReps} push-ups</p>
                    </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  The AI counts your reps in real-time. When you reach {targetReps} reps, it automatically stops and submits to backend.
                </p>
              </div>
            </div>
            <div className="flex gap-4">
